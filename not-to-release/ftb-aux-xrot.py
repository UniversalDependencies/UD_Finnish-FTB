'''In April 2013, undo remaining FinnTreeBank native auxiliary verb
convention in UD version by rotating the local trees at such
verbs. The auxiliary is raised to be the (local) main verb, the old
main NON-verb lowered to be its child, old child nodes (of the old
main verb) that precede the old auxiliary raised to the new main verb
in order to not introduce non-planar structure (maybe they even belong
there properly, who knows).

This is a copy of a script that dealt with AUX-aux-VERB, edited to
deal with AUX-aux-NOT-A-VERB where the cases appeared to involve a
copula as another AUX, or sometimes just the negative verb. Marking
the NOT-A-VERB as xcomp (when ccomp appeared to work for VERB) based
on a single example produced by an aging version of udpipe with TDT
model.
...
def rot(sentence):
    '''Rotate AUX and NOT-A-VERB so they become VERB and NOT-A-VERB; at
    this point, sentence consists of token records at positive
    indices, ID and HD numeric, ID the same as the index, so any
    token-range likes must be kept somewhere else.
    ...
    for k, token in enumerate(sentence):
        ...
        if ( token[UPOS] == 'AUX' and
            ...
            # are child to a VERB - hopefully rotate precisely these
            if not OLD[UPOS] == 'VERB':
                for sub in (
                    ...
                    #
                    # relation 'xcomp' (in contrast to ccomp for VERB)
                    OLD[HD], OLD[REL] = NEW[ID], 'xcomp'
            else:
                pass
